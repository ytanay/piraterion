...
def find_ideal_island(game,team):
  ...
  for island in optional_islands:
    ...
    if (size == len(team) or (size==len(team)-1 and len(team)>2 )) and arrival_in_time(game,team[0],island) \
      and (pirates_per_island[island.id]<=1 \
...
def my_team(game,pirate):
  ...
  for pirate1 in game.my_pirates():
    ...
    if len(team)<=2 and game.in_range(pirate,pirate1) and pirate!=pirate1 and\
        (not game.is_capturing(pirate1) or (game.is_capturing(pirate1) and current_target.turns_being_captured<10)):
...
def best_island_twos(game,pirate):
  ...
  team=[pirate]
  for pirate1 in game.my_pirates():
    if game.in_range(pirate,pirate1) and pirate1!=pirate:
      team.append(pirate1)
...
def best_island_indiv(game, pirate):
  ...
  for island in one:
    if ghosters[pirate.id]!=island.id :
      if (pirates_per_island[island.id] >= 2 and island.owner!=game.ME and ghosters[pirate.id]!=island.id)\
...
def is_good(game, pirate, directions):
  ...
  if (game.is_occupied(dest) or not game.is_passable(dest)) and game.distance(pirate,closest)>2:
    game.debug("direction not good %s"%pirate.id)
    return False
  return True
...
def ghost_routine(game, ghost_pirate):
  ...
  if ghost_pirate.is_cloaked :
    ...
    if islands != []:
      ...
      if abandoned_island(game) in islands and game.distance(ghost_pirate,closest)>8:
...
def finish_the_job(game,pirate):
  ...
  for target in targets:
    if enemy_defence(game,target)<len(my_team(game,pirate)) and game.distance(target,pirate.location)<7 and len(my_team(game,pirate))>1:
...
def run(game, pirate):
  ...
  for enemy in enemies:
    ...
    if  aerial_distance(game,pirate, enemy) < 6 and not game.is_capturing(enemy) and (power < anti_power or \
...
def size_of_group(game, island):
  ...
  for enemy in enemies:
    if enemy not in ignoreEnemies:
      ...
      if  aerial_distance(game,group[0], island) < 7:
...
def is_alone(game,pirate):
  for pirate1 in game.my_pirates():
    if pirate!=pirate1 and not game.in_range(pirate,pirate1):
      return True
